...
@software: PyCharm
@file: det_LSTNet_paras.py
@time: 2024/10/31 8:50
"""
import numpy as np
import time
...
def main():
    ...
    # lr_list = [0.005, 0.002, 0.0005]
    # lr_list = [0.005, 0.002, 0.0005, 0.0002]
    # lr_list = range(5,21)

    args.skip = 7
    # skip_list = np.array([4,6,8,12,16,20]) * 2  # skip <= seq_len-CNN_kernel
    # skip_list = range(1,47)

    args.highway_window = 10
    # highway_window_list = range(2, 13)

    args.hid_CNN = 256
    # hid_CNN_list = [32, 64, 128, 256]

    args.CNN_kernel = 4
    # CNN_kernel_list = range(2, 11)

    args.hid_RNN = 128
    # hid_RNN_list = [32, 64, 128, 256]

    args.hidSkip = 64
    # hidSkip_list = [32, 64, 128, 256]

    # for lr_value in lr_list:
    #     # args.lr = lr_value
    #     args.lr = lr_value * 0.0001
    #     print("执行lr为{}的训练测试".format(str(lr_value)))
    #
    #     train_model(args, train_val_dataset)

    # for skip_value in skip_list:
    #     args.skip = skip_value
    #     print("执行skip为{}的训练测试".format(str(skip_value)))
    #
    #     train_model(args, train_val_dataset)

    # for highway_window_value in highway_window_list:
    #     args.highway_window = highway_window_value
    #     print("执行highway_window为{}的训练测试".format(str(highway_window_value)))
    #
    #     train_model(args, train_val_dataset)

    # for hid_CNN_value in hid_CNN_list:
    #     args.hid_CNN = hid_CNN_value
    #     print("执行hid_CNN为{}的训练测试".format(str(hid_CNN_value)))
    #
    #     for CNN_kernel_value in CNN_kernel_list:
    #         args.CNN_kernel = CNN_kernel_value
    #         print("执行CNN_kernel为{}的训练测试".format(str(CNN_kernel_value)))
    #         train_model(args, train_val_dataset)

    # for hid_RNN_value in hid_RNN_list:
    #     args.hid_RNN = hid_RNN_value
    #     print("执行hid_RNN为{}的训练测试".format(str(hid_RNN_value)))
    #
    #     for hidSkip_value in hidSkip_list:
    #         args.hidSkip = hidSkip_value
    #         print("执行hidSkip为{}的训练测试".format(str(hidSkip_value)))
    #         train_model(args, train_val_dataset)
