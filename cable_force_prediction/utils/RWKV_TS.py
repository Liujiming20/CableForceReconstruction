...
class Model(nn.Module):
    ...
    def __init__(self, args, device):
        ...
        self.pred_len = args.pred_len  # 预测步长
        self.patch_size = args.patch_size
        self.stride = args.stride
        self.d_ff = args.d_ff
        self.patch_num = (self.seq_len - self.patch_size) // self.stride + 1

        self.padding_patch_layer = nn.ReplicationPad1d((0, self.stride))  # 使用 ReplicationPad1d 在右侧填充指定大小的重复片段
        self.patch_num += 1
        self.enc_embedding = DataEmbedding(args.enc_in * self.patch_size, args.d_model, args.embed, args.freq, args.dropout)

        rwkv_config = RWKVConfig(n_layer=args.gpt_layers,
                                 n_head=args.n_heads,
                                 n_embd=args.d_model)
        ...
        self.predict_linear = nn.Linear(self.patch_size, args.enc_in)
        self.ln = nn.LayerNorm(args.d_ff)
        self.out_layer = nn.Linear(args.d_ff, args.c_out)

        # for i, (name, param) in enumerate(self.rwkv.named_parameters()):
        ...
        for layer in (self.rwkv, self.predict_linear_pre, self.predict_linear, self.ln, self.out_layer):
    ...
    def forward(self, x_enc, x_mark_enc, x_dec, x_mark_dec):
        ...
        return dec_out[:, -self.pred_len:, -1:]  # 我们是预测最后一个维度——索力

        # return dec_out[:, -self.pred_len:, :]  # 不使用Nrom